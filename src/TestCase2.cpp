...
int main()
{
    SimulationParameter parameter;          // Initialization of Simulation parameters which contains methods for Boundary Conditions.

    size_t gridSizes[3] {65,129,257};       // Grid Sizes for given Square domain
    ...
    std::cout<<"Task2: Homogeneous Neumann boundary condition: "<<std::endl;
    ...
    for (size_t i = 0; i<3; i++) // Run the simulation for different number of grid points.
    {
        ...
        parameter.setLeftNeumannBC(dy, newTemp, oldTemp, gridSizes[i]); // Left BC
        ...
        // Solve and assign number iterations
        size_t iter = solve.solveIterNeumann(oldTemp, newTemp, tol, dx, dy, gridSizes[i], error, domainLength); 
        ...
        const std::string filename{"Heat_Conduction_Neumann_BC_" + size + "x" + size+".vtk"};
